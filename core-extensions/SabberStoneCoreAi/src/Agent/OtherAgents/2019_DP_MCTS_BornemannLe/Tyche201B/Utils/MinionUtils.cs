...
{
	internal static class MinionUtils
	{
		...
		private static void Init()
		{
			...
			for (var manaCost = 0; manaCost < NUM_MANA_TIERS; manaCost++)
				_estimatedFromMana[manaCost] = (manaCost + 1.0f) * 2.0f;

			var value = 0.0f;
			for (var manaCost = 0; manaCost < NUM_MANA_TIERS; manaCost++)
			{
				var count = manaCost + 1;
		...
		public static float ComputeMinionValue(int health, int attackDmg, int attacksPerTurn, bool hasTaunt = false,
			bool poisonous = false, bool hasDeathRattle = false, bool hasInspire = false, bool hasDivineShield = false,
			bool hasLifeSteal = false, bool hasCharge = false, bool hasStealh = false, bool hasBattleCry = false,
			bool isFrozen = false)
		{
			var value = 0.0f;
		...
		{
			var value = ComputeMinionValue(minion.Health, minion.AttackDamage, minion.NumAttacksThisTurn,
				minion.HasTaunt, minion.Poisonous, minion.HasDeathrattle, minion.HasInspire, minion.HasDivineShield,
				minion.HasLifeSteal, minion.HasCharge, minion.HasStealth, minion.HasBattleCry, minion.IsFrozen);
			return value;
		}
		...
		public static float ComputeMinionValues(Controller player)
		{
			...
			foreach (var t in player.BoardZone)
				value += ComputeMinionValue(t);
